...
void findCountours(cv::Mat& grayscale, vector<cv::RotatedRect>& result) {
	...
	*/
	...
	std::cout << "\nDuration of finding: " << duration_cast<milliseconds>(end - start).count() << std::endl;
...
void cut(cv::Mat& grayscale, vector<cv::Mat>& result) {
	...
	std::cout << "Duration of cutting: " << duration_cast<milliseconds>(end - start).count() << std::endl;
...
std::string analyse(cv::Mat& grayscale) {
	...
	if (finalCnts.size() <= 80 && finalCnts.size() > 50) {
		...
		*/
		...
		for (auto& cnt : finalCnts) {
			cv::rectangle(frame, cnt, cv::Scalar(255, 0, 0));
	...
	std::cout << "Duration of analyse: " << duration_cast<milliseconds>(end - start).count() << std::endl;
...
int main()
{
	...
	std::cout << "\nWork " << possibleImgs.size() << std::endl;
