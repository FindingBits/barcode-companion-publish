...
// 6 -> Unknown Prefix
// 7 -> Barcode not found
class BarcodeDecodingException : public std::exception
{
    int errnum;
    std::string message;

public:
     explicit BarcodeDecodingException(const char * msg) :message(msg)
    {
        errnum = 8;
    }
    explicit BarcodeDecodingException(std::string msg, int errnum_) : errnum(errnum_), message(msg)
    {
    }
    virtual ~BarcodeDecodingException() throw (){}

    const int getError() const { return errnum; }
...
string decimalToBinary(unsigned long long int N, size_t size)
...
GLF6 galoisAdd(GLF6 a, GLF6 b)
{ // addition in galois 2^6
	return static_cast<GLF6>(a ^ b);
}

GLF6 galoisMult(GLF6 x, GLF6 y)
{ // multiplication in galois 2^6
	if (x == 0 || y == 0)
		return 0;
	return (galoisExp[galoisLog[x] + galoisLog[y]]);
}

GLF6 galoisDiv(GLF6 x, GLF6 y)
{ // division in galois 2^6
	if (y == 0)
	{
		throw BarcodeDecodingException("Tried Galois Division by 0", 1);
	}
	if (x == 0)
		return 0;
...
GLF6 galoisPow(GLF6 x, int power)
{ // pow in galois 2^6
	return (galoisExp[(GLF6)trueMod((int)galoisLog[x] * power, 63)]);
}

GLF6 galoisInverse(GLF6 x)
{ // inverse in galois 2^6
	return (galoisExp[63 - galoisLog[x]]);
}

// if further debug needed use this function
void galoisPrint(GLF6 a)
{ // print number in galois 2^6
	int i = 8;
	while (i--)
		cout << ((a >> i & 1) + '0');
	cout << endl;
...
GLF6Polynomial galoisPolyAdd(const GLF6Polynomial& p, const GLF6Polynomial& q)
{
	...
	for (size_t i = 0; i < pSize; i++)
		r[i + rpDifference] = p[i];
	for (size_t i = 0; i < qSize; i++)
		r[i + rqDifference] ^= q[i];
...
void createTables()
{
	...
	for (int i = field - 1; i < (field * 2) - 1; i++)
		galoisExp[i] = galoisExp[i - 63];
...
void createPoly()
{
	...
	for (GLF6 i = 0; i < nroots; i++)
	{ //nroot iterations needed
		//first do coefficients $nroots down to 1
		for (GLF6 j = nroots; j > 0; j--)
		{
			g[j] = galoisAdd(g[j - 1], galoisMult(g[j], root));
		}
...
GLF6Polynomial rsCalcSyndromes(const GLF6Polynomial& msg, size_t nSym)
{
	...
	for (size_t i = 0; i < nSym; i++)
		synd[i] = galoisPolyEval(msg, galoisPow(alpha, i + initpwr));
...
GLF6Polynomial rsFindErrorLocator(const GLF6Polynomial& syndromes, const size_t nSym, int eraseCount)
{
	...
	if (syndromes.size() > nSym)
		syndShift = syndromes.size() - nSym;

	for (size_t i = 0; i < nSym - eraseCount; i++)
	...
	if ((errSize) * 2 > nSym)
	{
		throw BarcodeDecodingException("Too many errors to correct.", 2);
	}
...
GLF6Polynomial rsFindErrors(const GLF6Polynomial& errorLocator, const size_t messageSize)
{ // nmess is len(msg_in)
	...
	{
		if (galoisPolyEval(errorLocator, galoisPow(alpha, (int)i)) == 0)
...
GLF6Polynomial rsCorrectMessage(const GLF6Polynomial& message, GLF6 nSym, const GLF6Polynomial& erasurePositions, GLF6Polynomial& syndromes)
{
	...
	{
		throw BarcodeDecodingException("Too many erasures to correct", 3);
	}
	...
	{
		throw BarcodeDecodingException("Could not correct barcode errata.", 4);
	}
...
GLF6Polynomial findErasures(string& barcode) {
	GLF6Polynomial erasures;

	for (auto chara = barcode.begin(); chara != barcode.end(); chara++) {
		if ((*chara) == 'U') {
			(*chara) = 'F';

			GLF6 position = (chara - barcode.begin()) / 3;
			if (find(erasures.begin(), erasures.end(), position) == erasures.end()) {
				erasures.push_back(position);
			}
		}
	}
...
void reverseErasures(GLF6Polynomial& erasures, size_t messageSize) {
	for (auto erasure = erasures.begin(); erasure != erasures.end(); erasure++) {
		(*erasure) = messageSize - (*erasure) - 1;
	}
}
...
{
	struct OutputFormat* result = new struct OutputFormat[1];
	result->errnum = 0;

	string barcode(barcodePointer);

	try
	{
		if (barcode.length() != 75)
		{
			throw BarcodeDecodingException("Barcode size is wrong.", 5);
		}

		GLF6Polynomial erasures = findErasures(barcode);
		...
		{
			try {
				message = rsCorrectMessage(message, 12, erasures, syndromes); // the polynomial is where the errors are, adjust according to error positions

			}
			catch (BarcodeDecodingException ex) {
				string upsideDownBarcode(barcode.rbegin(), barcode.rend());

				for (auto chara = upsideDownBarcode.begin(); chara != upsideDownBarcode.end(); chara++) {
					if ((*chara) == 'A') (*chara) = 'D';
					else if ((*chara) == 'D') (*chara) = 'A';
				}
				message.clear();

				barsToPolynomial(upsideDownBarcode, 0, 30, message);
				barsToPolynomial(upsideDownBarcode, 66, 75, message);
				barsToPolynomial(upsideDownBarcode, 30, 66, message);

				reverseErasures(erasures, message.size());

				syndromes = rsCalcSyndromes(message, 12);
				if (!rsCheck(syndromes)) {
					message = rsCorrectMessage(message, 12, erasures, syndromes); // the polynomial is where the errors are, adjust according to error positions
				}
			}
		...
		{
			throw BarcodeDecodingException("[Unknown Prefix]", 6);
		}
		...
		for (string::size_type i = 0; i < equipmentIdentifier.length(); ++i)
			output << (char)toupper(equipmentIdentifier[i]);
		...
		size_t stringSize = finalOutput.length();

		result->output = new char[stringSize + 1];
		strcpy((*result).output, finalOutput.c_str());
	}
	catch (BarcodeDecodingException ex) {
		result->errnum = ex.getError();
